...
public abstract partial class Method
{
    public static partial class Ui
    {
        ...
        public static void Toast(string msg, WindowNotificationManager p_notification = null,
            ...
            string title = "Le Yu Minecraft Launcher")
        {
        ...
        public static async Task<(bool, bool, string, string)> CheckUpdateAsync()
        {
            try
            {
                var resourceName = "LeYMCL.Main.Public.Texts.DateTime.txt";
                ...
                var githubApiJson = JArray.Parse(await httpClient.GetStringAsync(Const.String.UpdateApiUrl));
        ...
        public static async Task<bool> UpdateAppAsync()
        {
            ...
            try
            {
                ...
                foreach (var jToken in assets)
                {
                    ...
                    switch (name)
                    {
                        case "LeYMCL.Main.linux.arm.AppImage" when architecture == "linux-arm":
                        case "LeYMCL.Main.linux.arm64.AppImage" when architecture == "linux-arm64":
                        case "LeYMCL.Main.linux.x64.AppImage" when architecture == "linux-x64":
                        case "LeYMCL.Main.osx.mac.x64.app.zip" when architecture == "osx-x64":
                        case "LeYMCL.Main.osx.mac.arm64.app.zip" when architecture == "osx-arm64":
                        case "LeYMCL.Main.win.x64.installer.exe"
                            when architecture == "win-x64" && Environment.OSVersion.Version.Major >= 10:
                        case "LeYMCL.Main.win.x86.installer.exe"
                            when architecture == "win-x86" && Environment.OSVersion.Version.Major >= 10:
                        case "LeYMCL.Main.win.arm64.installer.exe"
                            when architecture == "win-arm64" && Environment.OSVersion.Version.Major >= 10:
                        case "LeYMCL.Main.win7.x64.exe.zip"
                            when architecture == "win-x64" && Environment.OSVersion.Version.Major < 10:
                        case "LeYMCL.Main.win7.x86.exe.zip"
                            when architecture == "win-x86" && Environment.OSVersion.Version.Major < 10:
                        case "LeYMCL.Main.win7.arm64.exe.zip"
                            when architecture == "win-arm64" && Environment.OSVersion.Version.Major < 10:
